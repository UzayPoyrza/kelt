    ...
    <div className="min-h-screen" style={{ background: "var(--color-sand-50)" }}>
            ...
            {stage === "ready" && (
              <motion.div key="ready" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-xl mx-auto">
                ...
                <motion.button
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  onClick={() => { setStage("options"); setIsPlaying(false); setShowBgPicker(false); }}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-[var(--color-sand-600)] hover:text-[var(--color-sand-900)] hover:bg-white/60 border border-[var(--color-sand-200)] hover:border-[var(--color-sand-300)] transition-all cursor-pointer mb-6"
                  style={{ fontFamily: "var(--font-body)" }}
                >
                  <ChevronLeft className="w-4 h-4" />Back
                </motion.button>
                ...
                <div className="text-center mb-6">
                  <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-sm text-[var(--color-sand-500)] mb-2" style={{ fontFamily: "var(--font-body)" }}>Your meditation is ready</motion.p>
                  <motion.h1 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-2xl text-[var(--color-sand-900)] mb-3" style={{ fontFamily: "var(--font-display)" }}>&ldquo;{prompt}&rdquo;</motion.h1>
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-white rounded-2xl shadow-sm border border-[var(--color-sand-200)] overflow-hidden">
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between">
                      <button
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs transition-all cursor-pointer ${showBgPicker ? "bg-[var(--color-sand-900)] text-[var(--color-sand-50)]" : "text-[var(--color-sand-600)] hover:bg-[var(--color-sand-100)] border border-[var(--color-sand-200)]"}`}
                        style={{ fontFamily: "var(--font-body)" }}
                      >
                        <Music className="w-3.5 h-3.5" />
                        {soundscape ? `Background: ${soundscape}` : "Change background sound"}
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={handleStartOver} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-[var(--color-sand-600)] hover:bg-[var(--color-sand-100)] transition-colors text-xs cursor-pointer" style={{ fontFamily: "var(--font-body)" }}>
                  ...
                  <AnimatePresence>
                    {showBgPicker && (
                      ...
                      >
                        <div className="border-t border-[var(--color-sand-200)] px-6 py-4 space-y-3">
                          {/* Volume slider — always visible when sound selected */}
                          {soundscape && (
                            <div className="flex items-center gap-3">
                              <Volume2 className="w-3.5 h-3.5 text-[var(--color-sand-400)] shrink-0" />
                              <input
                                type="range" min="0" max="100" value={bgVolume}
                                onChange={(e) => setBgVolume(Number(e.target.value))}
                                className="flex-1 h-1 appearance-none bg-[var(--color-sand-200)] rounded-full cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[var(--color-sand-900)]"
                              />
                              <span className="text-[10px] text-[var(--color-sand-400)] w-8 text-right font-mono">{bgVolume}%</span>
                            </div>
                          )}

                          {/* Recommended — horizontal pills */}
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {(soundscapePresets[detectedIntent] || soundscapePresets.default).slice(0, 2).map((preset) => {
                                const isSel = soundscape === preset.label;
                                return (
                                  <button key={preset.label} onClick={() => setSoundscape(isSel ? null : preset.label)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs transition-all cursor-pointer ${isSel ? "bg-[var(--color-sand-900)] text-[var(--color-sand-50)] shadow-sm" : "bg-[var(--color-sand-50)] text-[var(--color-sand-700)] hover:bg-[var(--color-sand-100)] border border-[var(--color-sand-200)]"}`} style={{ fontFamily: "var(--font-body)" }}>
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {(soundscapePresets[detectedIntent] || soundscapePresets.default).slice(2).concat(
                                (soundscapePresets[detectedIntent === "default" ? "sleep" : "default"] || []).slice(0, 1)
                              ).map((preset) => {
                                const isSel = soundscape === preset.label;
                                return (
                                  <button key={preset.label} onClick={() => setSoundscape(isSel ? null : preset.label)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs transition-all cursor-pointer ${isSel ? "bg-[var(--color-sand-900)] text-[var(--color-sand-50)] shadow-sm" : "bg-[var(--color-sand-50)] text-[var(--color-sand-700)] hover:bg-[var(--color-sand-100)] border border-[var(--color-sand-200)]"}`} style={{ fontFamily: "var(--font-body)" }}>
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {Object.values(soundscapePresets).flat()
                                .filter((p, i, arr) => arr.findIndex((x) => x.label === p.label) === i)
                                .filter((p) => !(soundscapePresets[detectedIntent] || soundscapePresets.default).some((r) => r.label === p.label))
                                .slice(0, 4)
                                .map((preset) => {
                                  const isSel = soundscape === preset.label;
                                  return (
                              ...
                              {/* No background pill */}
                              <button onClick={() => setSoundscape(null)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs transition-all cursor-pointer ${!soundscape ? "bg-[var(--color-sand-200)] text-[var(--color-sand-900)]" : "bg-[var(--color-sand-50)] text-[var(--color-sand-500)] hover:bg-[var(--color-sand-100)] border border-[var(--color-sand-200)]"}`} style={{ fontFamily: "var(--font-body)" }}>
      ...
      <section className="relative py-24 px-6" style={{ background: "var(--color-sand-50)" }}>
        <div className="max-w-4xl mx-auto">
          ...
          {/* Interactive prompt → protocol demo */}
          <FadeIn>
