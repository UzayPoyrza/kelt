...
export default function HomePage() {
  ...
  const handleSamplePlay = useCallback((id: string, durationStr: string) => {
    ...
    const start = sample.sampleStart ?? 0;
    const limit = sample.sampleLimit ?? 30;
    ...
    // Progress tracking capped at per-sample limit
    const tick = 100;
    sampleIntervalRef.current = setInterval(() => {
      if (voiceAudioRef.current) {
        const elapsed = voiceAudioRef.current.currentTime - start;
        if (elapsed >= limit) {
          stopAudio();
          setPlaying(null);
          return;
        }
        setSampleProgress((p) => ({ ...p, [id]: elapsed / limit }));
      }
    }, tick);
